...
@section('content')
    <div class="max-w-md mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-bold mb-6">Modifier le Livre</h2>
                ...
                <div class="mb-4">
                    ...
                    <select name="id_author" class="w-full px-4 py-2 border rounded-lg" required>
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}" {{ $author->id == $book->id_author ? 'selected' : '' }}>
                                {{ $author->name }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @foreach($categories as $category)
                            <label class="flex items-center">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}" 
                                    {{ $book->categories->contains($category->id) ? 'checked' : '' }} class="mr-2">
